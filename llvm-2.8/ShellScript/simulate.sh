...
#--volTrace2ilp
ITRACE=$APP.trace
OUTPUT=$APP"_"$SCHEME"_"$BLOCK_SIZE"_"$CACHE_SIZE.statBase
...
#--SymbolTraceOpti
OUTPUT_OPT=$APP"_"$SCHEME"_"$BLOCK_SIZE"_"$CACHE_SIZE.statOpt
...
#OUTPUT=$APP.optiStat
	...
	#volTrace2ilp
	echo "$PIN -t $PIN_TOOL -hw $SCHEME -it $ITRACE -b $BLOCK_SIZE -c $CACHE_SIZE -ot $OTRACE -og $OGRAPH_ILP -o $OUTPUT -- ./$APP >log"
	$PIN -t $PIN_TOOL -hw $SCHEME -it $ITRACE -b $BLOCK_SIZE -c $CACHE_SIZE -ot $OTRACE -og $OGRAPH_ILP -o $OUTPUT -- ./$APP >log
	...
	if [ $SWITCH = "ilp" ]; then		
		...
		echo "$PIN -t $PIN_TOOL_OPT -hw $SCHEME -it $ITRACE -id $DATAMAP"_ilp.txt" -b $BLOCK_SIZE -c $CACHE_SIZE -o $OUTPUT_OPT_ILP  -- ./$APP >>log"
		$PIN -t $PIN_TOOL_OPT -hw $SCHEME -it $ITRACE -id $DATAMAP"_ilp.txt" -b $BLOCK_SIZE -c $CACHE_SIZE -o $OUTPUT_OPT_ILP  -- ./$APP >>log
	else
		...
		#from allocation.result to optiStat
		echo "$PIN -t $PIN_TOOL_OPT -hw $SCHEME -it $ITRACE -id $DATAMAP".txt" -b $BLOCK_SIZE -c $CACHE_SIZE -o $OUTPUT_OPT  -- ./$APP >>log"
		$PIN -t $PIN_TOOL_OPT -hw $SCHEME -it $ITRACE -id $DATAMAP".txt" -b $BLOCK_SIZE -c $CACHE_SIZE -o $OUTPUT_OPT  -- ./$APP >>log	
	fi
